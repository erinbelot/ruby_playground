...
Datatypes
If you have been having some trouble with ideas around datatypes and data structures, then here there might be a bit of a need here to do some forgetting. As much as that is possible. Ideally you can come at this without some assumptions that you have about all of this, and see it fresh.
...
3.2 - this is a float datatype, and its value is two point three.
...
Data Structures
Now we are going to talk about other constructs in coding, which I have been calling datatypes. These are more correctly describes as data structures. But the distinction is not always so clear. These are really constructs in coding - things made up by the programmers that came before you - to make shifting and mutating and combining those basic elements we have been discussion much easier. They are a means to an end. They are tools, and in some ways can be thought of in some ways as storage facilities.
Arrays are the structure that were made to store things that are similar. They come built into Ruby, and provide a place to store similar datatypes. They are helpful for storing data because they come indexed. By convention the first is 0, and the index goes up by 1 for every element added to the array.
...
Arrays are useful because they come ready made with indexes, and because we ‘know’ that they will contain the same type of data. Although we could fill them with different types of things, we try not to do this because it ruins their benefits. Nothing stops us from putting any mix of anything we like into an array. We could put in strings and floats and booleans into one array. But instantly we have lost the power of the structure - a structure that is there merely to help us. We have to program them to hold the same type of data so that when we loop through them they will behave as expected.
For example, let’s say we have an array filled with numbers that we want to add together and find the total, but a string sneaks in. Everything will go well until we try to add a string to our running total, at which point our code breaks. This is a problem for us, but it is of our own making. No point blaming the array.
...
Furthermore, in most languages arrays come with many helpful built in functions (methods in ruby) that give us ready made easy ways to use arrays. Once we have massaged our data so that we have the type of array that we want, we now have a set of other tools that can help us to look through this array. This is much more convenient than having a heap of variables.
That they contain the same datatype is really just the minimum requirement. Let’s say you have a list of students, but are storing both the first names and last names in the array. Maybe it looks like this:
...
In this case the data we are trying to represent could be first and last names, number of grand slams, number of kids, and age. Here, some of these are best represented as integers rather than strings. Now, you could potentially argue that we could just make them strings, and then we’d have the same datatype. But would this be much of an improvement?
[“Roger”, “Federer”, “20”, “36”, “8”, “4”]
If someone doesn’t know that “36” is his age, then what does that number mean to them?
The hash datatype helps because we have keys that describe the data we are looking at, and these keys have a value.
...
In the same way as we are all made from the same elements, but just in different combinations, all code ever is in those basic datatypes being pushed around all over the place in larger structures.
...
def print_array(array) # # defines a method called print_array that takes one argument, aan array
    array.each do |element|   # loop through the array, each element in turn is stored
            print element + ‘, ’      # print out each element, and add a comma and a space
    end # end of the loop
